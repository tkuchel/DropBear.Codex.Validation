...
public partial class CheckValidator : ICheckValidator
{
    ...
    {
        return !Uri.TryCreate(url.ToString(), UriKind.Absolute, out _)
            ? ValidationResult.Fail(
                new Dictionary<string, string>(StringComparer.Ordinal)
                {
                    { parameterName, $"{parameterName} cannot be null." }
    ...
    /// </summary>
    public static ValidationResult NotNull(object? value, string parameterName)
    {
        return value == null
            ? ValidationResult.Fail(
                new Dictionary<string, string>(StringComparer.Ordinal)
    ...
    /// </summary>
    public static ValidationResult IsPositive(int value, string parameterName)
    {
        return value > 0
            ? ValidationResult.Success()
            : ValidationResult.Fail(
    ...
    /// </summary>
    public static ValidationResult IsNegative(int value, string parameterName)
    {
        return value < 0
            ? ValidationResult.Success()
            : ValidationResult.Fail(
    ...
    {
        return (type.IsValueType && Activator.CreateInstance(type)!.Equals(value)) || value is null;
    }
